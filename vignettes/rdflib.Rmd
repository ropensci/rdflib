...
```{r}
vita <- rdf_parse("http://www.carlboettiger.info/js/vita.json", "jsonld")
...
sparql <-
 'PREFIX schema: <http://schema.org/>
  ...
  WHERE { 
    ...
    ?authors schema:givenName ?coi_given .
...
Let's compare that to extracting the same data with `jq` query: 

```{r}
library(jqr)
vita <- readr::read_file("http://www.carlboettiger.info/js/vita.json")
jq(vita,
   '."@reverse".author[]  | 
     { year: .dateCreated, 
