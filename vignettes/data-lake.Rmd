...
flights <- flights %>% 
  filter(distance > 2800) # try smaller dataset
}
```
...
The RDF approach merely treats JSON as JSON-LD within a given vocabulary.  In this context, nesting implicitly provides important semantic information about relationships between the data, which are captured in the RDF triples. Here, we import the JSON data as RDF:
...
We can add it to the existing data lake just for kicks:
