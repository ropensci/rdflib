...
gh_tibble %>% arrange(name)
```



```{r}
gh_rdf <- as_rdf.list(gh_data)
```

```{r}
rdf_serialize(gh_rdf, "test.json", "jsonld")
names(gh_tibble)
```
...
WHERE {
?repo <x:homepage>  ?homepage .
?repo <x:has_wiki> ?wiki .
?repo <x:open_issues_count> ?issues .
?repo <x:name> ?name .
?repo <x:owner> ?owner_id .
?owner_id <x:login>  ?owner 
}'
rdf_tibble <- rdf_query(rdf, s)
rdf_tibble %>% arrange(name) 
```
